...
notify_cmd_action="-A action1=Open -A action2=Delete"
...
notify_cmd_base="notify-send -t 10000 ${notify_cmd_action} -h string:x-canonical-private-synchronous:shot-notify"
...
notify_view() {
    if [[ "$1" == "active" ]]; then
        if [[ -e "${active_window_path}" ]] || ! $SAVE; then
...
countdown() {
	for sec in $(seq $1 -1 1); do
		notify-send -h string:x-canonical-private-synchronous:shot-notify -t 800 -i "$iDIR"/timer.png  " Taking shot" " in: $sec secs"
		sleep 1
	done
}
...
shotactive() {
    ...
    if $SAVE; then
        # Speichern UND in Zwischenablage
        grim -g "$region" - | tee "${active_window_path}" | wl-copy
    else
        # Nur in Zwischenablage, kein Speichern
        grim -g "$region" - | wl-copy
        active_window_path=""  # leer setzen, damit notify_view nicht auf Datei prüft
        notify_cmd_action=""
    fi
